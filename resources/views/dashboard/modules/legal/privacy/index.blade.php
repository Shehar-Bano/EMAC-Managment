<x-dashboard.layout :title="'Privacy Policy — EMAC Development ERP'">

    <x-slot:breadcrumbs>
        <span class="text-slate-400">/</span>
        <span class="font-semibold text-slate-800">Privacy Policy</span>
    </x-slot:breadcrumbs>

    <x-slot:header>
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Privacy Policy Management</h1>
            <p class="text-xs text-slate-500 mt-1">Manage customer data protection standards, privacy disclosures, and compliance terms</p>
        </div>

        <div class="flex items-center gap-2.5">
            <a href="{{ route('privacy') }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 shadow-xs transition-colors">
            ...
            @can('privacy.edit')
                <x-button href="{{ route('dashboard.privacy.edit') }}" variant="primary" size="sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <x-card title="Document Metadata" subtitle="Version & publication status">
                <div class="space-y-4 text-xs">
                    <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                        ...
                        <span class="font-bold text-slate-800 uppercase">Privacy Policy</span>
                    </div>
                    ...
                    <div class="pt-2">
                        <div class="p-3 bg-amber-50/60 rounded-xl border border-amber-200/70 text-[11px] text-[#8F6B20]">
                            <strong>API Integration:</strong> This content is instantly syndicated to the public website and mobile API endpoint (<code>GET /api/v1/privacy</code>).
                        </div>
                    </div>
                </div>
            </x-card>
        ...
        <div class="lg:col-span-2">
            <x-card title="{{ $document->title ?? 'Privacy Policy' }}" subtitle="Live preview of privacy disclosures">
