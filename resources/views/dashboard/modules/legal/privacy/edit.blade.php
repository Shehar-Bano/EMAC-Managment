<x-dashboard.layout :title="'Edit Privacy Policy — EMAC Development ERP'">

    <x-slot:breadcrumbs>
        <span class="text-slate-400">/</span>
        <a href="{{ route('dashboard.privacy.index') }}" class="text-slate-500 hover:text-slate-700">Privacy Policy</a>
        <span class="text-slate-400">/</span>
    ...
    <x-slot:header>
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Privacy Policy</h1>
            <p class="text-xs text-slate-500 mt-1">Update data collection policies, compliance rules, and publication status with rich text formatting</p>
        </div>

        <div class="flex items-center gap-2.5">
            <x-button href="{{ route('dashboard.privacy.index') }}" variant="secondary" size="sm">
                Cancel
            </x-button>
        </div>
    </x-slot:header>
    ...
    <div class="max-w-5xl">
        <form method="POST" action="{{ route('dashboard.privacy.update') }}" id="legal-form">
            @csrf
            @method('PUT')

            <x-card title="Privacy Policy Content" subtitle="Update privacy policy language and metadata">
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">
                            Privacy Policy Content (Rich Text Editor) <span class="text-rose-500">*</span>
                        </label>
                        <textarea
                            name="content"
                            id="editor"
                    ...
                    <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                        <x-button href="{{ route('dashboard.privacy.index') }}" variant="secondary">
                            Cancel
                        </x-button>
