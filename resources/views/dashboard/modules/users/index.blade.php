<x-dashboard.layout :title="'User Management — EMAC Development ERP'">
    ...
    <x-slot:breadcrumbs>
        ...
        <span class="font-semibold text-slate-800">Users Directory</span>
    </x-slot:breadcrumbs>

    <x-slot:header>
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Staff User Management</h1>
        ...
        >
            @forelse ($users as $user)
                <tr class="hover:bg-slate-50/70 transition-colors">
                    ...
                    {{-- Sr. No --}}
                    <td class="w-16 px-6 py-3.5 font-mono text-xs text-slate-500 font-semibold">
                    ...
                    <td class="px-6 py-3.5">
                        <div class="flex items-center gap-3">
                            <img src="{{ $user->avatar_url }}" alt="{{ $user->name }}" class="w-9 h-9 rounded-xl object-cover ring-2 ring-[#C5A059]/30 shadow-2xs shrink-0">
                            <div>
                                <a href="{{ route('dashboard.users.show', $user) }}" class="font-bold text-slate-900 hover:text-[#C5A059] transition-colors">
                                    {{ $user->name }}
                                </a>
                    ...
                    <td class="px-6 py-3.5">
                        <div class="flex flex-wrap gap-1">
                            @forelse ($user->roles as $role)
                                <span class="px-2 py-0.5 rounded-md text-[11px] font-bold {{ $role->slug === 'super-admin' ? 'bg-slate-900 text-[#D4AF37]' : 'bg-[#C5A059]/10 text-[#8F6B20] border border-[#C5A059]/20' }}">
                                    {{ $role->name }}
                                </span>
                            @empty
                    ...
                    <td class="px-6 py-3.5 text-right">
                        <x-action-dropdown>
                            <a href="{{ route('dashboard.users.show', $user) }}" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-[#C5A059]/10 hover:text-[#B8903B] transition-colors">
                            ...
                            @can('users.edit')
                                <a href="{{ route('dashboard.users.edit', $user) }}" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-[#C5A059]/10 hover:text-[#B8903B] transition-colors">
                            ...
                            @can('users.status')
                                @if (auth()->id() !== $user->id)
                                    <form method="POST" action="{{ route('dashboard.users.status', $user) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="w-full flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-amber-50 hover:text-amber-600 transition-colors cursor-pointer text-left">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-sm font-semibold text-slate-700">No staff members found</p>
                            <p class="text-xs text-slate-400 mt-1">Try resetting search filters or add a new user.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
