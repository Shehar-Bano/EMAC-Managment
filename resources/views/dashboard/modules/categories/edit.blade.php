<x-dashboard.layout :title="'Edit Category — EMAC Development ERP'">

    <x-slot:breadcrumbs>
        <span class="text-slate-400">/</span>
        <a href="{{ route('dashboard.categories.index') }}" class="hover:text-[#C5A059]">Categories</a>
        <span class="text-slate-400">/</span>
        <span class="font-semibold text-slate-800">Edit {{ $category->name }}</span>
    </x-slot:breadcrumbs>

    <x-slot:header>
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Category</h1>
            <p class="text-xs text-slate-500 mt-1">Update classification parameters, visual banner, and publication status</p>
        </div>
            ...
            <x-card title="Category Information & Visual Image" subtitle="Category ID: #{{ $category->id }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
                    ...
                    <div>
                        <x-select label="Publication Status" name="status" required>
                            <option value="active" {{ old('status', $category->status) === 'active' ? 'selected' : '' }}>Active (Public & Listed)</option>
                            <option value="inactive" {{ old('status', $category->status) === 'inactive' ? 'selected' : '' }}>Inactive / Draft</option>
                        </x-select>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <x-input
                        ...
                        hint="Leave empty to re-sync slug with category name."
                    />
                </div>
