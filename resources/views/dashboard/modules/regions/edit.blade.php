<x-dashboard.layout :title="'Edit Region — ' . $region->name . ' — EMAC Development ERP'">

    <x-slot:breadcrumbs>
        <span class="text-slate-400">/</span>
        <a href="{{ route('dashboard.regions.index') }}" class="hover:text-[#C5A059]">Regions</a>
        <span class="text-slate-400">/</span>
        <span class="font-semibold text-slate-800">Edit Region: {{ $region->name }}</span>
    </x-slot:breadcrumbs>

    <x-slot:header>
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Service Region</h1>
            <p class="text-xs text-slate-500 mt-1">Modify region parameters, active status, code, or local currency</p>
        </div>

        <div class="flex items-center gap-2">
            <x-button href="{{ route('dashboard.regions.show', $region) }}" variant="secondary" size="sm">
                View Region
            </x-button>
    ...
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('dashboard.regions.update', $region) }}" class="space-y-6">
            @csrf
            @method('PUT')

            <x-card title="Region Details" subtitle="Update geographical territory and commercial configurations">
                <div class="space-y-4">
                    {{-- Name --}}
                    <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                name="currency"
                                value="{{ old('currency', $region->currency) }}"
                                maxlength="10"
                                class="w-full px-3.5 py-2 text-xs bg-white border border-slate-300 rounded-lg focus:border-[#C5A059] focus:ring-1 focus:ring-[#C5A059] outline-none uppercase font-mono transition-colors"
                            >
                            @error('currency')
